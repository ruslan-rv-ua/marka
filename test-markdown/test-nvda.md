...
## Тестування анонсів при відкритті файлу

1. **Завантаження з командного рядка (CLI):** 
   - Запустіть програму з аргументом: `marka.exe test-markdown\test-nvda.md`
   - Перевірте, чи NVDA одразу після старту озвучує `"Відкрито: test-nvda.md"` (або "Opened: test-nvda.md" в залежності від мови системи).
2. **Відкриття через Ctrl+O:**
   - Натисніть `Ctrl+O`, оберіть інший файл.
   - Перевірте, чи NVDA озвучив назву нового файлу.
3. **Навігація по історії (Alt+Left / Alt+Right):**
   - Натисніть `Alt+Left`. NVDA має озвучити назву попереднього файлу.
   - Натисніть `Alt+Right`. NVDA має озвучити назву поточного файлу.
